...
const CommentsContainer = () => {
    ...
    let { blog, setBlog, blog: { _id, title, comments: { results: commentsArr }, activity: { total_parent_comments } }, commentsWrapper, setCommentsWrapper, totalParentCommentsLoaded, setTotalParentCommentsLoaded } = useContext(BlogContext)
    ...
    return(
        <div className={"max-sm:w-full fixed " + ( commentsWrapper ? "top-0 sm:right-0" : "top-[100%] sm:right-[-100%]" ) + " duration-700 max-sm:right-0 sm:top-0 w-[30%] min-w-[350px] h-full z-50 bg-white shadow-2xl p-8 px-16 overflow-y-auto overflow-x-hidden"}>
            ...
            <button onClick={() => setCommentsWrapper(preVal => !preVal)} className="absolute top-4 right-4 flex justify-center items-center w-12 h-12 rounded-full bg-grey">
